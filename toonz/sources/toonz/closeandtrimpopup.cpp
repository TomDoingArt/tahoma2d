...
//**************************************************************************
//    Clean And Trim Swatch
//**************************************************************************

class CloseAndTrimPopup::Swatch final : public PlaneViewer {
  TVectorImageP m_vi;
...
CloseAndTrimPopup::FrameData::FrameData() : m_frameIdx(-1) {}

//--------------------------------------------------------------

CloseAndTrimPopup::FrameData::FrameData(const TXshSimpleLevelP &sl,
                                           int frameIdx)
    : m_sl(sl), m_frameIdx(frameIdx) {}

//--------------------------------------------------------------

CloseAndTrimPopup::FrameData CloseAndTrimPopup::FrameData::getCurrent() {
...
bool CloseAndTrimPopup::FrameData::operator==(const FrameData &other) const {
  return (m_sl == other.m_sl) && (m_frameIdx == other.m_frameIdx);
}

//--------------------------------------------------------------

TVectorImageP CloseAndTrimPopup::FrameData::image() const {
...
CloseAndTrimPopup::SelectionData::SelectionData()
    : m_contentType(NONE), m_sl() {}

//--------------------------------------------------------------

CloseAndTrimPopup::SelectionData::SelectionData(const TSelection *sel)
    : m_contentType(NONE), m_sl() {
  struct Locals {
...
CloseAndTrimPopup::SelectionData::SelectionData(const FrameData &fd)
    : m_contentType(IMAGE)
...
CloseAndTrimPopup::SelectionData::operator bool() const {
  return (m_contentType != NONE);
}

//--------------------------------------------------------------

void CloseAndTrimPopup::SelectionData::getRange(int &startIdx,
                                                   int &endIdx) const {
...
//**************************************************************************
//    Close And Trim  operation
//**************************************************************************

namespace {

typedef CloseAndTrimPopup::SelectionData SelectionData;
...
//**************************************************************************
//    CleanAndTrimPopup implementation
//**************************************************************************

CloseAndTrimPopup::CloseAndTrimPopup()
    : DVGui::Dialog(TApp::instance()->getMainWindow(), true, false,
                    "CloseAndTrim")
    , m_validPreview(false) {
  setWindowTitle(tr("Close And Trim"));
  setLabelWidth(0);
  setModal(false);
  ...
  topLayout->addWidget(new QLabel(tr("Max Gap Distance:")), row, 1, Qt::AlignRight);
  //m_vectorGapDistanceValue = new DVGui::MeasuredDoubleField;
  //topLayout->addWidget(m_vectorGapDistanceValue, row, 2, Qt::AlignRight);
  //m_vectorGapDistance = new QSlider(Qt::Horizontal,this);
  //m_vectorGapDistance = new DVGui::MeasuredDoubleField(this); //QSlider(Qt::Horizontal, this);
  DVGui::IntField *m_vectorGapDistance = new DVGui::IntField(this);
  //m_vectorGapDistance->setMinimum(1);
  //m_vectorGapDistance->setMaximum(100);
  
  /*m_vectorGapDistance->setRange(1.0,100.0);
  m_vectorGapDistance->setFixedHeight(14);
  m_vectorGapDistance->setValue(50);
  */
  m_vectorGapDistance->enableSlider(true);
  //m_vectorGapDistance->setSizeIncrement(QSize::width);

  //m_vectorGapDistanceValue->setValue(50);

  topLayout->addWidget(m_vectorGapDistance, row++, 2, Qt::AlignLeft);
  ...
  topLayout->setRowStretch(2, 0);  // Needed to justify at top
  ...
  //m_viewer->setMinimumHeight(350); // TDA - was 150 originally
  //m_viewer->setMaximumHeight(350); // TDA - added this call to max height
...
void CloseAndTrimPopup::showEvent(QShowEvent *se) {
...
void CloseAndTrimPopup::hideEvent(QHideEvent *he) {
  Dialog::hideEvent(he);
...
void CloseAndTrimPopup::updateSelectionData() {
  m_selectionData =
...
void CloseAndTrimPopup::updateCurrentFrameData() {
  m_currentFrameData = FrameData::getCurrent();
}

//--------------------------------------------------------------

void CloseAndTrimPopup::updateSelectionGui() {
...
void CloseAndTrimPopup::onModeChanged() {
...
void CloseAndTrimPopup::onXsheetChanged() {
  updateSelectionData();
...
void CloseAndTrimPopup::onSelectionChanged() {
  updateSelectionData();
...
void CloseAndTrimPopup::onFrameChanged() {
  updateCurrentFrameData();
...
void CloseAndTrimPopup::onParamsChanged() { schedulePreviewUpdate(); }

//--------------------------------------------------------------

void CloseAndTrimPopup::schedulePreviewUpdate() {
  m_validPreview = false;
...
void CloseAndTrimPopup::getTransformParameters(double (&fromTransform)[2],
...
void CloseAndTrimPopup::updatePreview() {
  if (!m_validPreview) {
    m_validPreview = true;
...
class CloseAndTrimUndo final : public TUndo {
public:
  CloseAndTrimUndo(const SelectionData &selData, double (&fromTransform)[2],
                      double (&toTransform)[2]);
...
CloseAndTrimUndo::CloseAndTrimUndo(const SelectionData &selData,
...
void CloseAndTrimUndo::redo() const {
...
void CloseAndTrimUndo::undo() const {
...
void CloseAndTrimPopup::apply() {
  ...
  std::unique_ptr<TUndo> undo(
      new CloseAndTrimUndo(m_selectionData, fromTransform, toTransform));
...
OpenPopupCommandHandler<CloseAndTrimPopup> openCloseAndTrimPopup(
    MI_CloseAndTrim);
