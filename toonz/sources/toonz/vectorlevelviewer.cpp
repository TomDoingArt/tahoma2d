...
//
//   std::ofstream file("C:\\temp\\VectorLevelViewerStrokes.txt");
//
//   //Header
...
//       //VectorLevelViewPane::getVectorLineData(file, i, v[i]->m_groupId.m_id);
//   }
//   //printStrokes(file);
//   file.close();
//
...
//-----------------------------------------------------------------------------
VectorLevelViewerFrame::VectorLevelViewerFrame(QScrollArea *parent,
                                              Qt::WindowFlags flags)
   : QFrame(parent, flags), m_scrollArea(parent) {
 setObjectName("VectorLevelViewerFrame");
...
VectorLevelViewPane::VectorLevelViewPane(QWidget *parent, Qt::WindowFlags flags)
   : QWidget(parent) {
 ...
 //create a new QTree instance.

  //m_frameArea->setObjectName("VectorLevelViewerFrameArea");
  //m_frameArea->setFrameStyle(QFrame::StyledPanel);
  //m_frameArea->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOn);
  //m_frameArea->verticalScrollBar()->setObjectName("VectorLevelViewerFrameAreaScrollBar");
  //m_frameArea->setHorizontalScrollBarPolicy(Qt::ScrollBarAsNeeded);
 ...
 connect(proxyView, &QWidget::customContextMenuRequested, this, &VectorLevelViewPane::showContextMenu);
 ...
 connect(filterPatternLineEdit, &QLineEdit::textChanged,
         this, &VectorLevelViewPane::filterRegExpChanged);
 ...
 filterPatternLineEdit->setText(""); //set default filter here, like: "0|1" to filter on value 0 or 1
...
void VectorLevelViewPane::onLevelChanged() {
...
void VectorLevelViewPane::showEvent(QShowEvent *) {
  ...
  connect(TApp::instance()->getCurrentFrame(), 
          SIGNAL(frameSwitched()), this, SLOT(onLevelChanged()));
...
void VectorLevelViewPane::hideEvent(QHideEvent *) {
...
void VectorLevelViewPane::setSourceModel(QAbstractItemModel *model)
{
   proxyModel->setSourceModel(model);
}

void VectorLevelViewPane::filterRegExpChanged()
{
...
void VectorLevelViewPane::filterColumnChanged()
{
   proxyModel->setFilterKeyColumn(filterColumnComboBox->currentIndex());
}

void VectorLevelViewPane::sortChanged()
{
...
void VectorLevelViewPane::copySelectedItemsToClipboard() {
...
void VectorLevelViewPane::contextMenuEvent(QContextMenuEvent *event) {
   QMenu menu(this);
   QAction *copyAction = menu.addAction(tr("Copy"));
   connect(copyAction, &QAction::triggered, this, &VectorLevelViewPane::copySelectedItemsToClipboard);

   // Show the menu at the cursor position
   menu.exec(event->globalPos());
}

void VectorLevelViewPane::showContextMenu(const QPoint &pos) {
